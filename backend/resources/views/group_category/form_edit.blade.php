        ...
        <div class="max-w-7xl mx-auto px-4 space-y-4">
            <div class="bg-white overflow-auto shadow-sm sm:rounded-lg">
                <div class="p-4 text-gray-900">
                    ...
                    <form action="{{route('group_category.update', ['id'=>$group_category->id])}}" method="post" class="mt-4 space-y-4">
                        @csrf
                        @method('PATCH')
                        <div class="space-y-1">
                            <label for="name" class="text-sm font-medium">Group Category Name</label>
                            <input type="text" class="w-full rounded-lg" name="name" id="name" value="{{$group_category->name}}" placeholder="Name...">
                            @error('name')
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="text" class="w-full rounded-lg" name="code" id="code" value="{{$group_category->code}}" placeholder="Code...">
                            @error('code')
                            <p class="text-sm text-red-500 font-medium">{{$message}}</p>
                            @enderror
                        </div>
