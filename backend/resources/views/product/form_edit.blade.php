        ...
        <div class="max-w-7xl mx-auto px-4 space-y-4">
            <div class="bg-white overflow-auto shadow-sm sm:rounded-lg">
                <div class="p-4 text-gray-900">
                    ...
                    @enderror
                    <form action="{{route('product.update', ['id' => $product->id])}}" enctype="multipart/form-data" method="post" class="mt-4 space-y-4">
                        @csrf
                        @method('PATCH')
                        <div class="space-y-1">
                            <label for="name" class="text-sm font-medium">Product Name</label>
                            <input type="text" class="w-full rounded-lg" name="name" id="name" placeholder="Name..." value="{{old('name') ?? $product->name}}">
                            @error('name')
                        ...
                        <div class="space-y-1">
                            ...
                            <input type="text" class="w-full rounded-lg" name="code" id="code" placeholder="Code..." value="{{old('code') ?? $product->code}}">
                            @error('code')
                            <p class="text-sm text-red-500 font-medium">{{$message}}</p>
                            @enderror
                        </div>
                        <div class="space-y-1">
                            ...
                            <textarea class="form-textarea w-full rounded-lg" name="description" id="description" placeholder="Description...">{{old('description') ?? $product->description}}</textarea>
                            @error('description')
                            <p class="text-sm text-red-500 font-medium">{{$message}}</p>
                            @enderror
                        </div>
                        <div class="space-y-1">
                            ...
                            <select class="form-multiselect w-full rounded-lg" name="category[]" id="category" multiple>
                                ...
                                <option value="{{$category->id}}" @if(in_array($category->id, $product_category['id'])) selected @endif>{{$category->name}}</option>
                                @endforeach
                            </select>
                            @error('category')
                            <p class="text-sm text-red-500 font-medium">{{$message}}</p>
                        ...
                        <div class="space-y-1">
                            ...
                            <select class="form-multiselect w-full rounded-lg" name="content_management[]" id="content_management" multiple>
                                ...
                                <option value="{{$cm->id}}" @if(in_array($cm->id, $product_cm['id'])) selected @endif>{{$cm->name}}</option>
                                @endforeach
                            </select>
